...
//#include <avr/power.h>
//#endif
#define PinLedsTauler 52
#define PinLedsBandera 53
#define numLedsT 40
...
const int sensorsBanderaI[10] = { 24, 26, 28, 30, 32, 34, 36, 38, 40, 42 };  //sensors de de descobrir les caselles i de col·locar banderes
      ...
      if (valorTaul[numPin] == LOW && mapaAccionsJug[numPin] == 0) {  //Si és LOW i aquella casella no te cap bandera col·locada ni s'ha descobert
        switch (mapaJoc[numPin]) {                                    //Depenent del valor del mapejat del joc es realitza alguna de les seguents accions
          case -1:
            ...
            break;
          case 0:
            color = verd;  //No hi ha cap gat a prop
            break;
          case 1:
            color = blau;  //Hi ha un gat a prop
            break;
          case 2:
            color = taronja;  //Hi ha dos gats a prop
            break;
          case 3:
  ...
  if (canviTauler == true) {
    ...
    for (int numPin = 0; numPin < numSensors; numPin++) {
      for (int i = 0; i < 10; i++) {  //Per a tots els sectors de la tira de LEDs
        int led = i * 4;
        uint32_t colorLed = colorLedsT[i];  //cada sector esta compost per 4 leds
        if (colorLedsT[i] == gris) {
          pixelsT.setPixelColor(led, pixelsT.Color(negre));
          pixelsT.setPixelColor(led + 1, pixelsT.Color(negre));
          pixelsT.setPixelColor(led + 2, pixelsT.Color(negre));
          pixelsT.setPixelColor(led + 3, pixelsT.Color(colorLed));
        }
        //S'agafa el color del que s'ha de pintar aquella part de la tira de leds
        else {
          pixelsT.setPixelColor(led, pixelsT.Color(negre));
...
void Generador() {
  ...
  for (int i = 0; i < 40; i++) {
    ...
    if ((i - 3) % 4 == 0) {
      pixelsT.setPixelColor(i, gris);
    } else {
      pixelsT.setPixelColor(i, negre);  //Per als LEDs del tauler de joc, nomes es pinten de color gris els que el seu numero no es multiple de 2
    }
  }
  for (int k = 0; k < 20; k++) {
    if (k < 10) {
  ...
  for (int i = 0; i < 10; i++) {
    valorTaul[i] = HIGH;
    valorBand[i] = HIGH;  //Llistes que contenen els valors actuals del que detecten els sensors
    valorTaulAnt[i] = HIGH;
    valorBandAnt[i] = HIGH;  //Llistes que contenen els valors antics del que han detectat els senosrs
  }
