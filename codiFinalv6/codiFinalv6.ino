...
void setup() {
  ...
  pixelsT.begin();
  pixelsB.begin();
  pixelsT.setBrightness(100);
  pixelsB.setBrightness(100);  //S'inicialitzen les dues tires de LEDs i es defineix la seva brillantor
...
void Guanyar() {  //Animació de victoria
  ...
  for (int v = 0; v < 5; v++) {  //durant 5 cops
    for (int t = 0; t < pixelsT.numPixels(); t++) {
      pixelsT.setPixelColor(t, pixelsT.Color(verd));
    }
    ...
    delay(500);
    for (int t = 0; t < pixelsT.numPixels(); t++) {
      pixelsT.setPixelColor(t, pixelsT.Color(blau));
    }
    for (int b = 0; b < pixelsB.numPixels(); b++) {
...
void Perdre() {  //Animacio de perdre
  ...
  for (int v = 0; v < 5; v++) {
    for (int t = 0; t < pixelsT.numPixels(); t++) {
      pixelsT.setPixelColor(t, pixelsT.Color(vermell));
    }
    for (int b = 0; b < pixelsB.numPixels(); b++) {
    ...
    delay(500);
    for (int s = 0; s < pixelsT.numPixels(); s++) {
      pixelsT.setPixelColor(s, pixelsT.Color(taronja));
    }
    for (int s = 0; s < pixelsB.numPixels(); s++) {
      pixelsB.setPixelColor(s, pixelsB.Color(taronja));
    }
    pixelsT.show();
